...
def crear_mapa(df, mostrar_nombres=False):
    mapa = folium.Map(
        location=[29.0892, -110.9613],
        zoom_start=9,
        tiles="OpenStreetMap"
    )
    ...
    for _, row in df.iterrows():
        ...
        # Punto pequeño para que se vean TODAS las visitas sin encimarse.
        folium.CircleMarker(
            location=[lat, lon],
            radius=6,
            color=color,
        ...
        # Etiqueta opcional. Si se prende, muestra nombres; si no, queda limpio para presentación.
        if mostrar_nombres:
            etiqueta = (
                f'<div style="font-size:9pt;color:{color};'
    ...
    if puntos_validos:
        mapa.fit_bounds(puntos_validos, padding=(45, 45))

    return mapa


def mapa_html_bytes(df, mostrar_nombres=False):
    mapa = crear_mapa(df, mostrar_nombres=mostrar_nombres)
...
# =====================================================
    ...
    if df.empty:
        ...
        mostrar_nombres = st.checkbox(
            ...
            value=False,
            help="Para presentación se recomienda dejarlo apagado para que se vean todos los puntos."
        )

        st_folium(crear_mapa(df_mapa, mostrar_nombres=mostrar_nombres), width=None, height=650)

        col1, col2 = st.columns(2)
        with col1:
            st.download_button(
                "🗺️ Descargar mapa interactivo HTML",
                data=mapa_html_bytes(df_mapa, mostrar_nombres=mostrar_nombres),
                file_name="Mapa_Escuelas_Visitadas_DIF.html",
                mime="text/html"
            )
        with col2:
            st.info("Para presentación: deja apagado 'Mostrar nombres', descarga el HTML, abre pantalla completa y toma captura con Windows + Shift + S.")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if df.empty:
        ...
        with col3:
            st.download_button(
                ...
                data=mapa_html_bytes(df, mostrar_nombres=False),
                file_name="Mapa_Escuelas_Visitadas_DIF.html",
                mime="text/html"
            )
