...
def conectar_firebase():
    ...
    try:
        if "firebase" in st.secrets:
            ...
            firebase_admin.initialize_app(cred, {"projectId": firebase_config.get("project_id", "dif-hermosillo")})
            return firestore.client()
...
def cargar_datos():
    ...
    try:
        # Usamos get(timeout=10) para evitar que Streamlit se quede pensando indefinidamente.
        docs = coleccion.limit(LIMITE_REGISTROS).get(timeout=10)
        ...
        st.info("Firebase ya conecta, pero la consulta a Firestore está fallando o tardando demasiado.")
        return pd.DataFrame(columns=columnas_base)

    if not registros:
        st.warning(f"Firebase conectó, pero no encontró registros en la colección: {COLECCION}")
        return pd.DataFrame(columns=columnas_base)

    df = pd.DataFrame(registros)

    # Normalizar columnas por si algún documento tiene mayúsculas u otros nombres.
