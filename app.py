    ...
    try:
        if "firebase" in st.secrets:
            ...
            firebase_admin.initialize_app(
                cred,
                {"projectId": firebase_config.get("project_id", "dif-hermosillo")}
            )
            return firestore.client()
...
def cargar_datos():
    ...
    try:
        # Lectura controlada para que la página no se quede pensando.
        docs = db.collection(COLECCION).limit(25).get(timeout=8)
    ...
    return pd.DataFrame(registros, columns=columnas_base)
...
if "df_cache_manual" not in st.session_state:
    st.session_state.df_cache_manual = pd.DataFrame(columns=["id", "nombre", "nivel", "lat", "lon"])

if st.button("🔄 Cargar datos de Firebase"):
    with st.spinner("Leyendo Firestore..."):
        st.session_state.df_cache_manual = cargar_datos()

df = st.session_state.df_cache_manual
