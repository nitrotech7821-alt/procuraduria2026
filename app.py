...
def conectar_firebase():
    """
    Conexión local a Firebase usando archivo JSON.
    IMPORTANTE:
    - Coloca el archivo JSON en la misma carpeta donde está app.py.
    - No usa secrets.toml en local para evitar errores de StreamlitSecretNotFoundError.
    """
    ...
    st.error(
        "No se encontró el archivo JSON de Firebase. "
        f"Coloca este archivo junto a app.py: {JSON_LOCAL}"
    )
    return None
...
def cargar_datos():
    ...
    registros = []
    try:
        for doc in coleccion.limit(LIMITE_REGISTROS).stream():
            d = doc.to_dict()
            d["id"] = doc.id
            registros.append(d)
    except Exception as e:
